#!/usr/bin/env bash
# -------------------------------------------------------------------
# add-service-dns.sh — Register a new *.home.local service in all DNS locations
#
# Usage:
...
#
# What it does:
#   1. Adds address=/<subdomain>.home.local/<SERVER_IP> to Pi-hole dnsmasq config
#   2. Adds <SERVER_IP> <subdomain>.home.local to /etc/hosts (for Twingate)
...
SERVER_IP="10.10.2.200"
PIHOLE_CONF="$(dirname "$0")/../../pihole/dnsmasq.d/05-home-local.conf"
HOSTS_FILE="/etc/hosts"

# Resolve relative path
PIHOLE_CONF="$(cd "$(dirname "$PIHOLE_CONF")" && pwd)/$(basename "$PIHOLE_CONF")"

usage() {
    echo "Usage: sudo bash $0 <subdomain> [port] [--reload]"
    echo ""
    echo "Arguments:"
    echo "  subdomain   The subdomain name (e.g., 'grafana' → grafana.home.local)"
